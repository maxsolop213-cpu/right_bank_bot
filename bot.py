...
def generate_photo_stats_text():
    ...
    for uid, data in sorted_data:
        ...
        text += f"⏰ Почав: {times[0] if times else '-'} | Завершив: {times[-1] if times else '-'} | 🕐 ~{avg_interval} хв\n"
...
def remark_handler(message):
    ...
    uid = str(photo_msg.from_user.id)
...
def manual_check_foto(message):
    if not is_tm_or_admin(message.from_user.id):
        return
    bot.send_message(message.chat.id, generate_photo_stats_text())
...
def photo_group_scheduler():
    ...
    while True:
        ...
        if now.weekday() <= 4:
            ...
            # 10:00 — нагадування хто не скинув
            ...
            # щоп’ятниці 18:00 — підсумок тижня
