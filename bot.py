...
creds = Credentials.from_service_account_info(
    json.loads(GOOGLE_CREDENTIALS), scopes=scope)
client = gspread.authorize(creds)
sheet = client.open_by_key(MAIN_SHEET_ID)
users_ws = sheet.worksheet("Users")
photo_ws = sheet.worksheet("PhotoStats")

# 🔹 Створення аркуша для зауважень, якщо
# його нема
try:
  remarks_ws = sheet.worksheet("PhotoRemarks")
except gspread.exceptions.WorksheetNotFound:
  remarks_ws = sheet.add_worksheet(
    title="PhotoRemarks", rows=100, cols=4)
...
def is_tm_or_admin(user_id):
  ...
  or user_id in TM_IDS or user_id == ADMIN_ID
 )
...
def start(message):
  ...
  user = get_user_data(user_id)
   if not user:
  bot.reply_to(
    message, "�️ Тебе немає в списку користувачів. Звернись до керівника.")
   return

  name = user.get("Ім’я", "користувач")
  bot.send_message(
    message.chat.id, f"👋 Привіт, {name}! {
        random.choice(MOTIVATION_DAILY)}")
...
@bot.message_handler(content_types=["photo",
                                    "document",
                                    "text"],
                     func=lambda m: m.chat.id == PHOTO_GROUP_ID)
...
def generate_photo_stats_text():
  ...
  avg_interval = 0
   if len(times) > 1:
  fmt = "%H:%M:%S"
  diffs = [(datetime.strptime(t2,
                                                    fmt) - datetime.strptime(t1,
                                                                             fmt)).seconds for t1,
                                 t2 in zip(times,
                                           times[1:])]
...
text += f"⏰ Почав: {times[0] if times else '-'} | Завершив: {times[-1] if times else '-'}\n"
  ...
  missing = [
    u["Ім’я"]   for u in all_users   if str(u.get("Telegram_ID", "")).strip().isdigit()   and str(u["Telegram_ID"]) not in sent_users   and str(u.get("�оль", "")).lower() not in excluded_roles
 ]
   if missing:
...
def save_photo_stats_to_sheet():
  ...
  int((sum(
 (datetime.strptime(t2, "%H:%M:%S") - datetime.strptime(t1, "%H:%M:%S")).seconds   for t1, t2 in zip(times, times[1:])
...
def remark_handler(message):
  ...
  bot.reply_to(
    message, "�️ Відповідай на фото, до якого хочеш додати зауваження.")
  ...
  remark_text = message.text.replace(
    "/remark", "").strip() or "(Без тексту)"
...
@bot.message_handler(func=lambda msg: msg.text ==
                     "📊 Check Foto" or msg.text == "/check_foto")
...
def photo_group_scheduler():
  ...
  bot.send_message(
    PHOTO_GROUP_ID, "📸 Доброго ранку! Очікую ваші фото та коди 💪")
  ...
  excluded_roles = {"св", "sv", "tm", "тм"}
  missing = [
    u["Ім’я"]   for u in all_users
    if str(u.get("Telegram_ID", "")).strip().isdigit()   and str(u["Telegram_ID"]) not in sent_users   and str(u.get("�оль", "")).lower() not in excluded_roles
 ]
   if missing:
  bot.send_message(
    PHOTO_GROUP_ID, f"📸 Не бачу фото від: {
        ', '.join(missing)}")
  ...
  bot.send_message(
    PHOTO_GROUP_ID, "📅 Підсумок тижня готується...")
  send_photo_stats()
...
def handle_links(message):
  ...
  bot.send_message(
    message.chat.id,
    f"⛔️ Для '{column}' ще немає посилання.")
   return
