...
def start(message):
    ...
    bot.reply_to(
        message,
        "️ Тебе немає в списку користувачів. Звернись до керівника.")
    return
...
def handle_photo_group_message(message):
    ...
    if dt.startswith("image/") or re.search(
        r"\.(jpg|jpeg|png|webp|gif|bmp|heic)$",
        fname,
            flags=re.IGNORECASE):
    photo_data[uid]["photos"] += 1
    counted_image = True
...
def generate_photo_stats_text():
    ...
    if len(times) > 1:
    fmt = "%H:%M:%S"
    diffs = [(datetime.strptime(t2, fmt) - datetime.strptime(t1, fmt)
              ).seconds for t1, t2 in zip(times, times[1:])]
...
def remark_handler(message):
    ...
    bot.reply_to(
        message,
        "️ Відповідай на фото, до якого хочеш додати зауваження.")
...
def photo_group_scheduler():
    ...
    bot.send_message(
        PHOTO_GROUP_ID,
        "📸 Доброго ранку! Очікую ваші фото та коди 💪")
